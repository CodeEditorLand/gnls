...
function getRange(range: gn.Range): ls.Range {
	return {
		start: getPosition(range.begin),
			...
			: { line: range.begin.line, character: 0 },
	};
}
...
function getDiagnostics(file: string): ls.Diagnostic[] {
	...
	if (error) {
		result.push({
			range: getRange(
				error.ranges[0] ?? { begin: error.location, end: null },
			),
...
function getFunctionCompletion(name: string): ls.CompletionItem {
	...
	const result = {
		label: name,
		kind: detail.isTarget
			? ls.CompletionItemKind.Class
			: ls.CompletionItemKind.Function,
...
function getVariableCompletion(name: string): ls.CompletionItem {
	...
	const result = {
		label: name,
		kind: detail.isBuiltin
			? ls.CompletionItemKind.Variable
			: ls.CompletionItemKind.Field,
	} as ls.CompletionItem;
	if (help) {
		result.detail = help.basic;
		result.documentation = {
	...
	switch (context?.token?.type) {
		case "literal": {
			...
			if (context.token.value.startsWith('"')) {
				if (detail.isInput) {
					const string = context.token.value.substring(
						1,
						column - context.token.range.begin.column,
					);
					const parts = string.replace(/^\/+/, "").split(":", 2);
					const colon = parts.length > 1;
					const base = string.startsWith("//")
						? context.root
						: string.startsWith("/")
							? "/"
							: path.dirname(file);
					const relative = colon
						? parts[0]
						: parts[0].replace(/[^/]+$/, "");
						...
						if (colon) {
							if (detail.isLabel) {
								...
								scope?.declares.forEach((declare) => {
									...
									const target =
										func == "target" ? arg0 : func;
									const label =
										func == "target" ? arg1 : arg0;
									if (
										data.functionDetail(target).isTarget &&
										label
									) {
							...
							entries.forEach((entry) => {
								if (entry.isDirectory()) {
									result.push(
										getDirectoryCompletion(entry.name),
									);
		...
		default: {
			...
			if (context?.function) {
				...
				const arg0 = (context.function.arguments[0] ?? "").replace(
					/^"|"$/g,
					"",
				);
				...
				try {
					...
					if (entry.isFile()) {
						...
						const declare = scope?.declares.find((declare) => {
							const func = declare.function;
							const arg0 = (declare.arguments[0] ?? "").replace(
								/^"|"$/g,
								"",
							);
							const arg1 = (declare.arguments[1] ?? "").replace(
								/^"|"$/g,
								"",
							);
						...
						result.push(
							linkWithRange(
								filepath,
								declare ? getRange(declare.range) : undefined,
							),
						);
					}
				} catch {
...
function getDocumentSymbol(file: string): ls.DocumentSymbol[] {
	const mapToDocumentSymbol = (
		symbol: gn.GNDocumentSymbol,
	): ls.DocumentSymbol => {
