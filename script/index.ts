...
function ensure(deps: string) {
	const data = JSON.parse(
		fs.readFileSync(canonicalize(deps), { encoding: "utf8" }),
	) as {
		name: string;
		repo: string;
		commit: string;
...
function addon(debug: boolean, arch: string) {
	...
	copy(
		`build/${debug ? "Debug" : "Release"}/addon.node`,
		`../build/${os.platform()}-${arch}.node`,
	);
}
	...
	switch (target) {
		...
		case "build":
			...
			if (os.platform() == "darwin") {
				...
				console.warn(
					"Addon cross compiling is only implemented on macOS 11 for now.",
				);
				addon(false, os.arch());
			}
			break;
		case "debug":
			...
			if (os.platform() != "win32") {
				...
				console.warn(
					"Addon debug build is not working on Windows yet. Fallback to release build.",
				);
				addon(false, os.arch());
			}
			break;
